...
import { ArrowLeft, Upload, AlertTriangle, Save, Car, MapPin, AlignLeft, User, Calendar, Clock, X } from 'lucide-react';
...
export default function NewAlertScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let lat = mapCoords?.lat || (46.603354 + (Math.random() * 2 - 1));
      let lng = mapCoords?.lng || (1.888334 + (Math.random() * 2 - 1));
  ...
  return (
    <AdminLayout activeTab="new-alert">
        ...
        <div className="w-full max-w-3xl bg-[#111] border border-safe-border rounded-3xl overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg md:text-xl font-bold flex items-center gap-2">
                  <AlertTriangle className="text-safe-red" size={24} /> 
                  Déclencher une Alerte
                </h1>
                <p className="text-xs md:text-sm text-gray-400">Diffusion immédiate sur le réseau</p>
              </div>
            </div>
          </header>
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-10">
            ...
            <div className="pt-6 pb-8 border-t border-safe-border">
              ...
              <p className="text-center text-xs text-gray-500 mt-4 leading-relaxed max-w-lg mx-auto">
                En cliquant sur "Diffuser l'Alerte", une notification push silencieuse sera envoyée immédiatement aux utilisateurs dans le périmètre concerné.
              </p>
            </div>
          </form>
        </div>
